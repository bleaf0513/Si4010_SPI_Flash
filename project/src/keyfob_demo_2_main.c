...
#define BIN_CHUNK 64
xdata BYTE binChunk[BIN_CHUNK] ;  // Use external data for large arrays
...
void vStl_StartTx(void);
void vStl_StopTx(void);
/*------------------------------------------------------------------------------
...
void output_bin_data(void)
{
    ...
    while (remaining)
    {
        ...
        // Now send the data using the output function, e.g., vStl_ByteTx()
        vStl_ByteTx(binChunk, sendSize);  // Send the data over the output
